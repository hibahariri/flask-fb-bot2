...
def get_CartItem(recipientID):
    ...
    cur = con[0].cursor()
    cur.execute(
        "Select ItemDesc,CONCAT(size,' ',sizeunit),CONCAT(price,' LBP'), Itemimage, Quantity from cart inner join item where cart.itemID = item.itemID and userID = (Select userid from user where recipientID = %s) ",
        (recipientID.strip(),))
    records = cur.fetchall()
    cur.close()
    con[0].close()
