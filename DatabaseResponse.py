...
def get_Orders(recipientID):
    ...
    cur = con[0].cursor()
    cur.execute(
        "select OrderID, orderStatus,FORMAT(Total,0),DATE_FORMAT(creatDate,'%d %b %y'),CASE WHEN orderStatus = 0 THEN 'Open'  WHEN orderStatus = 1 THEN 'Processing' ELSE 'Delivered'"
