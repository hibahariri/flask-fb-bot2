...
def get_CartItem(recipientID):
    ...
    cur = con[0].cursor()
    cur.execute(
        "Select ItemDesc,CONCAT(size,' ',sizeunit),price, Itemimage, Quantity, (price * Quantity),CartID from cart inner join item where cart.itemID = item.itemID and userID = (Select userid from user where recipientID = %s) and Isdeleted ='yes' ",
        (recipientID.strip(),))
    records = cur.fetchall()
    cur.close()
    con[0].close()
